-- FiveM Tools Complete Database Schema
...
CREATE TABLE IF NOT EXISTS forum_threads (
  ...
  last_reply_by TEXT REFERENCES users(discord_id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS reports (
  ...
  description TEXT,
  reporter_id TEXT NOT NULL REFERENCES users(discord_id) ON DELETE CASCADE,
  status TEXT DEFAULT 'pending' CHECK (status IN ('pending', 'reviewing', 'resolved', 'dismissed')),
  moderator_id TEXT REFERENCES users(discord_id) ON DELETE SET NULL,
  moderator_notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  resolved_at TIMESTAMPTZ
);

-- Messages table (for future DM feature)
CREATE TABLE IF NOT EXISTS messages (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  sender_id TEXT NOT NULL REFERENCES users(discord_id) ON DELETE CASCADE,
  receiver_id TEXT NOT NULL REFERENCES users(discord_id) ON DELETE CASCADE,
  content TEXT NOT NULL,
  is_read BOOLEAN DEFAULT FALSE,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Create helper function for incrementing thread count
CREATE OR REPLACE FUNCTION increment_thread_count(cat_id UUID)
RETURNS void AS $$
BEGIN
...
-- Create helper function for incrementing reply count
CREATE OR REPLACE FUNCTION increment_reply_count(thread_uuid UUID)
RETURNS void AS $$
BEGIN
